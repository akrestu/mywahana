...
import { Download, Lock, Search, Trash2, Wrench } from 'lucide-react';
...
export default function AdminInspeksiWorkshop({ records, filters, summary, sites }: Props) {
...
return;
 ...
 setDeleting(false); setToDelete(null); 
    ...
    const exportUrl = `/admin/inspeksi-workshop/export?${new URLSearchParams(
        Object.fromEntries(Object.entries(filters).filter(([, v]) => v)) as Record<string, string>
    ).toString()}`;

    return (
        <>
            <Head title="Monitoring Inspeksi Workshop" />
            <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between gap-3">
                    <div>
                        <h2 className="text-xl font-bold">Inspeksi Workshop</h2>
            ...
            <Dialog open={!!toDelete} onOpenChange={() => setToDelete(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Yakin ingin menghapus data Inspeksi Workshop milik <strong>{toDelete?.user.name}</strong> tanggal{' '}
            ...
            <Dialog open={showBatchConfirm} onOpenChange={(open) => !open && setShowBatchConfirm(false)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Hapus {selectedIds.size} Data Inspeksi Workshop</DialogTitle>
                        <DialogDescription>
            ...
            <DeleteRangeDialog
                ...
                endpoint="/admin/inspeksi-workshop/delete-range"
                title="Hapus Data Inspeksi Workshop"
                description="Ini akan menghapus permanen seluruh data Inspeksi Workshop pada rentang tanggal yang dipilih (mengikuti batas site admin, di luar filter tampilan saat ini). Tindakan ini tidak dapat dibatalkan."
            />
        </>
    );
}
