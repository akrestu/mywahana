...
export default function AdminInspeksiMess({ records, filters, summary, sites }: Props) {
...
setSelectedIds(new Set(records.data.map(r => r.id)));
 ...
 setSelectMode(false); setSelectedIds(new Set()); 
    ...
    const handleBatchDelete = () => {
        ...
        router.delete('/admin/inspeksi-mess/batch', {
...
delete (merged as Record<string, unknown>)[k];
        ...
        router.get('/admin/inspeksi-mess', merged, { preserveState: true, replace: true });
    };
...
return;
        ...
        setDeleting(true);
        router.delete(`/admin/inspeksi-mess/${toDelete.id}`, {
            onFinish: () => {
 setDeleting(false); setToDelete(null); 
    ...
    const exportUrl = `/admin/inspeksi-mess/export?${new URLSearchParams(
        Object.fromEntries(Object.entries(filters).filter(([, v]) => v)) as Record<string, string>
    ).toString()}`;

    return (
        <>
            <Head title="Monitoring Inspeksi Mess" />
            <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between gap-3">
                    <div>
                        <h2 className="text-xl font-bold">Inspeksi Mess</h2>
            ...
            <Dialog open={!!toDelete} onOpenChange={() => setToDelete(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Yakin ingin menghapus data Inspeksi Mess milik <strong>{toDelete?.user.name}</strong> tanggal{' '}
            ...
            <Dialog open={showBatchConfirm} onOpenChange={(open) => !open && setShowBatchConfirm(false)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Hapus {selectedIds.size} Data Inspeksi Mess</DialogTitle>
                        <DialogDescription>
