...
export default function InspeksiTambangCreate({ user, staffUsers, sites }: Props) {
    ...
    const { data, setData, post, processing, errors } = useForm<FormData>({ re_inspektor_id: '', peserta_ids: [], tanggal: new Date().toISOString().split('T')[0], project_site: defaultSite, departemen: user.departemen ?? '', ...initialScores, tindakan_perbaikan: [], ttd_inspektor: '' });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post('/sap/inspeksi-tambang', { data: fd as unknown as FormData });
    };

    return (
        <>
            <Head title="Buat Inspeksi Tambang" />
            <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-w-2xl">
                <div>
                    <h2 className="text-xl font-bold">Inspeksi Area Tambang</h2>
                    <p className="text-sm text-muted-foreground mt-0.5">WBK-HSE-FO-005 — Langkah {step} dari 3</p>
                    ...
                    <div className="flex flex-col gap-5">
                        <Card className="p-0 overflow-hidden">
                            ...
                            <CardContent className="flex flex-col gap-4 py-4">
                                <div className="space-y-1.5"><Label>Tanggal <span className="text-destructive">*</span></Label><div className="relative"><Input type="date" value={data.tanggal} onChange={e => setData('tanggal', e.target.value)} className="h-12 text-base pr-10 [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:inset-0 [&::-webkit-calendar-picker-indicator]:w-full [&::-webkit-calendar-picker-indicator]:cursor-pointer" /><Calendar className="absolute right-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" /></div>{errors.tanggal && <p className="text-sm text-destructive">{errors.tanggal}</p>}</div>
