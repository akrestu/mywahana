...
import { CalendarDays, ChevronRight, Plus, ShieldAlert, Wrench } from 'lucide-react';
...
function RecordList({ records, asRI = false }: { records: Paginated; asRI?: boolean }) {
    ...
    if (!records.data.length) return (
        <div className="flex flex-col items-center gap-4 rounded-2xl border-2 border-dashed py-12 text-center px-6">
            <div className="flex size-16 items-center justify-center rounded-full bg-muted"><Wrench size={28} className="text-muted-foreground" /></div>
            ...
            {grouped.map(([month, items]) => (
                <div key={month} className="flex flex-col gap-3">
                    ...
                    <Card className="overflow-hidden p-0">
                        {items.map((rec, idx) => (
                            <div key={rec.id}>
                                <Link href={`/sap/inspeksi-workshop/${rec.id}`}>
...
export default function InspeksiWorkshopIndex({ myRecords, pendingReInspeksi, selesaiAsRI }: Props) {
    return (
        <>
            <Head title="Inspeksi Workshop" />
            <div className="flex flex-col gap-6">
                <div className="flex items-start justify-between gap-3">
                    <div><h2 className="text-xl font-bold">Inspeksi Area Workshop</h2><p className="text-sm text-muted-foreground mt-0.5">WBK-HSE-FO-007</p></div>
                    <Link href="/sap/inspeksi-workshop/create" className="shrink-0"><Button className="h-12 px-5 gap-2 text-base font-bold"><Plus size={18} /> Buat Inspeksi</Button></Link>
                </div>
