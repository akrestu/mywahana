...
type Record = { id: number; user: UserInfo; re_inspektor: UserInfo | null; tanggal: string; project_site: string; departemen: string; persentase: number | null; risk_level: 'L' | 'M' | 'H' | 'VH' | null };
type Props = { record: Record };

const RISK_CFG = { L: { label: 'Baik', cls: 'bg-green-100 text-green-800 border-green-300' }, M: { label: 'Cukup', cls: 'bg-yellow-100 text-yellow-800 border-yellow-300' }, H: { label: 'Perhatian', cls: 'bg-orange-100 text-orange-800 border-orange-300' }, VH: { label: 'Perlu Tindakan', cls: 'bg-red-100 text-red-800 border-red-300' } };
...
export default function InspeksiWorkshopReInspeksi({ record }: Props) {
    ...
    return (
        <>
            <Head title="Re-Inspeksi Workshop" />
            <div className="flex flex-col gap-6 max-w-2xl">
                {step === 'review' && (
                    <>
                        <div><h2 className="text-xl font-bold">Re-Inspeksi Area Workshop</h2><p className="text-sm text-muted-foreground mt-0.5">{tanggal}</p></div>
                        ...
                        {risk && record.persentase !== null && <div className={`rounded-2xl border-2 px-4 py-4 ${risk.cls}`}><div className="flex items-center justify-between"><p className="font-bold text-2xl">{record.persentase}%</p><span className="font-bold text-sm">{risk.label}</span></div></div>}
                        <div className="rounded-2xl border bg-blue-50/50 dark:bg-blue-950/10 border-blue-200 dark:border-blue-800 px-4 py-4"><p className="text-sm leading-relaxed text-blue-800 dark:text-blue-300">Sebagai re-inspektor, Anda diminta untuk memverifikasi hasil inspeksi ini. Lihat detail lengkap di halaman <a href={`/sap/inspeksi-workshop/${record.id}`} className="font-bold underline" target="_blank" rel="noreferrer">detail inspeksi</a>.</p></div>
                        <div className="flex flex-col gap-3 pt-2">
                            <p className="text-sm font-bold text-center text-muted-foreground">Keputusan Anda sebagai Re-Inspektor</p>
                            <div className="grid grid-cols-2 gap-3">
                                <Button variant="destructive" className="h-14 text-base font-bold gap-2" onClick={() => setStep('reject')}><XCircle size={20} /> Tolak</Button>
                                <Button className="h-14 text-base font-bold gap-2 bg-green-600 hover:bg-green-700" onClick={() => setStep('sign')}><CheckCircle2 size={20} /> Konfirmasi</Button>
                            </div>
                        </div>
                    </>
                )}
                ...
                {step === 'sign' && (
                    <form onSubmit={e => { e.preventDefault(); if (!hasSig) return; signForm.post(`/sap/inspeksi-workshop/${record.id}/re-inspeksi`); }} className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-3"><p className="text-base font-bold">Tanda Tangan <span className="text-destructive">*</span></p><SignaturePad onCapture={d => { signForm.setData('ttd_re_inspektor', d ?? ''); setHasSig(!!d); }} /></div>
                        <div className="grid grid-cols-2 gap-3">
                ...
                {step === 'reject' && (
                    <form onSubmit={e => { e.preventDefault(); if (!rejectForm.data.alasan.trim()) return; rejectForm.post(`/sap/inspeksi-workshop/${record.id}/tolak`); }} className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-2"><label className="text-base font-bold">Alasan Penolakan <span className="text-destructive">*</span></label><Textarea value={rejectForm.data.alasan} onChange={e => rejectForm.setData('alasan', e.target.value)} placeholder="Tuliskan alasan penolakan..." className="min-h-32 resize-none text-sm" /></div>
                        <div className="grid grid-cols-2 gap-3">
                            <Button type="button" variant="outline" className="h-14 text-base font-bold" onClick={() => setStep('review')}><ArrowLeft size={18} className="mr-2" /> Kembali</Button>
                            <Button type="submit" variant="destructive" className="h-14 text-base font-bold gap-2" disabled={rejectForm.processing || !rejectForm.data.alasan.trim()}><XCircle size={20} /> Kirim Penolakan</Button>
                        </div>
                    </form>
                )}
            </div>
