...
type Record = { id: number; user: UserInfo; re_inspektor: UserInfo | null; tanggal: string; project_site: string; lokasi: string; persentase: number | null; risk_level: 'L' | 'M' | 'H' | 'VH' | null };
...
export default function InspeksiMessReInspeksi({ record }: Props) {
    ...
    return (
        <>
            <Head title="Re-Inspeksi Mess" />
            <div className="flex flex-col gap-6 max-w-2xl">
                {step === 'review' && (
                    <>
                        <div><h2 className="text-xl font-bold">Re-Inspeksi Area Mess</h2><p className="text-sm text-muted-foreground mt-0.5">{tanggal}</p></div>
                ...
                {step === 'sign' && (
                    <form onSubmit={e => { e.preventDefault(); if (!hasSig) return; signForm.post(`/sap/inspeksi-mess/${record.id}/re-inspeksi`); }} className="flex flex-col gap-6">
                ...
                {step === 'reject' && (
                    <form onSubmit={e => { e.preventDefault(); if (!rejectForm.data.alasan.trim()) return; rejectForm.post(`/sap/inspeksi-mess/${record.id}/tolak`); }} className="flex flex-col gap-6">
